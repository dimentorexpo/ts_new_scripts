        ...
        <span style="width: 700px">
                <span style="cursor: -webkit-grab;">
                        <div style="margin: 5px; width: 700px;" id="naborData">
                                <button class="buttonHide" id="hideNaborStatus">hide</button>
                                <button class="btnCRM" id="openTrmTeacher" title="Очищает поля с результатами и полем для ввода">🧑‍🏫 TRM</button>
                        </div>
                        <div style="margin: 5px; width: 700px" id="databoNabor">
                            <input class="inputCRM" id="tidNabor" placeholder="Teacher ID" title="Введите ID учителя, чтобы проверить информацию по статусу набора" autocomplete="off" type="text" style="position:relative; text-align:center; width:100px; color:black; margin-left:40%;">
                            <button class="btnCRM" title="Запускает процесс поиска информации по статусам набора" id="getNaborInfo">🔍</button>
						</div>
				</span>
						<div>
...
let btnTNabor = document.getElementById('butTeacherNaborCRM');
btnTNabor.addEventListener('click', function () {
    ...
    if (document.getElementById('AF_NaborStatus').style.display == '') {
		...
		btnTNabor.classList.add('activeScriptBtn')
    }
})	
